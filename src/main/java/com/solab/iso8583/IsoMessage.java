...
public class IsoMessage {
    ...
    public String toCupsAsciiHexMessageWithOutHeader() {
        StringBuilder sb = new StringBuilder();
        // Add MTI
        sb.append(str2ascii(String.format("%04x", type)));
        // Bitmap by hex
        BitSet bs = createBitmapBitSet();
        int pos = 0;
        int lim = bs.size() / 4;
        ...
        for (int i = 2; i < 129; i++) {
            ...
            if (v != null) {
                ...
                if (v.getType() == IsoType.LLBIN || v.getType() == IsoType.LLBCDBIN || v.getType() == IsoType.LLVAR) {
                    len = String.format("%02d", desc.length());
                    String ascLen = str2ascii(len);
//                    System.out.println("len:" + len + ",asc:" + ascLen);
                    sb.append(ascLen);
                } else if (v.getType() == IsoType.LLLBIN || v.getType() == IsoType.LLLBCDBIN || v.getType() == IsoType.LLLVAR) {
                    len = String.format("%03d", desc.length());
                    String ascLen = str2ascii(len);
//                    System.out.println("len:" + len + ",asc:" + ascLen);
                    sb.append(ascLen);
                } else if (v.getType() == IsoType.LLLLBIN || v.getType() == IsoType.LLLLBCDBIN || v.getType() == IsoType.LLLLVAR) {
                    len = String.format("%04d", desc.length());
                    String ascLen = str2ascii(len);
//                    System.out.println("len:" + len + ",asc:" + ascLen);
                    sb.append(ascLen);
                }
                sb.append(str2ascii(desc));
            }
        }
        return sb.toString();
    }

    private String str2ascii(String str) {
