...
public class MessageFactory<T extends IsoMessage> {
    ...
    /**
     * Returns a custom field encoder/decoder for the specified field number, if one is available.@param
     */
     ...
     * but is really convenient in case the MessageFactory is being configured from within, say, Spring. @param @throws
     */
    ...
    public IsoValue<?> parseField(int type, int field, byte[] buf, int pos) throws UnsupportedEncodingException, ParseException {
        FieldParseInfo fpi = parseMap.get(type).get(field);
        return fpi.parse(field, buf, pos, fpi.getDecoder());
    }
